...
class CrossModalAttentionFusion(nn.Module):
    """
    Cross-Modal Attention Fusion Module (CMAFM) as described in the BMVC 2026 manuscript.

    This module performs bi-directional information exchange between visible (RGB)
    and thermal modalities using channel-wise cross-attention, spatial cross-gating,
    and a final gated fusion stage.

    Complexity: O(C) channel complexity, bypassing expensive spatial cross-attention.
    """
    ...
    def __init__(self, channels: int, num_heads: int = 8, dropout: float = 0.1):
        ...
        # Maps to Eq. (167) and Eq. (168) in the manuscript:
        # q_r = W_q * GAP(F_r)
        # k_t, v_t = split(W_kv * GAP(F_t))
        ...
        # --- Spatial cross-attention (DWConv + Conv 1x1) ---
        # Maps to Eq. (196) and Eq. (197) in the manuscript:
        # S_r = Conv_1x1(DWConv_3x3(F'_r))
        # S_t = Conv_1x1(DWConv_3x3(F'_t))
        ...
        # Cross spatial gates: Eq. (198) and Eq. (199):
        # G_r = Sigmoid(Conv_1x1([S_r; S_t]))
        # G_t = Sigmoid(Conv_1x1([S_t; S_r]))
        ...
        self.norm_rgb = nn.GroupNorm(NUM_GROUPS, channels)
        self.norm_thermal = nn.GroupNorm(NUM_GROUPS, channels)

        # Gated fusion: Eq. (211) and Eq. (212):
        # alpha = Sigmoid(Conv_1x1([F''_r; F''_t]))
        ...
        # Output projection: Eq. (215) and Eq. (216) (Conv 3x3 + Residual):
        # F_out = Conv_3x3(F_fused) + F_fused
    ...
    def _channel_cross_attn(self, q_proj: nn.Module, kv_proj: nn.Module,
                            feat_q: torch.Tensor, feat_kv: torch.Tensor) -> torch.Tensor:
        """
        Computes channel cross-attention: GAP -> project -> dot-product scale.

        Corresponds to Eq. (167) - (171) in the manuscript:
          q_r = W_q * GAP(F_r)
          k_t, v_t = split(W_kv * GAP(F_t))
          scale = Sigmoid(q_r * k_t)
          out = v_t * scale
        """
        B, C, _, _ = feat_q.shape

        q_vec = feat_q.mean(dim=[2, 3])   # B, C (Global Average Pooling)
        kv_vec = feat_kv.mean(dim=[2, 3])  # B, C

        q = q_proj(q_vec)                 # B, C (Eq. 167)
        kv = kv_proj(kv_vec)              # B, C*2 (Eq. 168)
        k, v = kv.chunk(2, dim=-1)        # B, C each

        # Element-wise product scaling (Eq. 170)
        scale = (q * k).sigmoid()         # B, C
        out = v * scale                   # B, C (Eq. 172 scale factor)
        return out.view(B, C, 1, 1)

    def forward(self, rgb_feat: torch.Tensor, thermal_feat: torch.Tensor) -> torch.Tensor:
        """
        Bidirectional forward pass for CMAFM.

        Inputs:
            rgb_feat: Visible features (B, C, H, W)
            thermal_feat: Infrared features (B, C, H, W)

        Returns:
            Fused and enhanced features (B, C, H, W)
        """
        B, C, H, W = rgb_feat.shape

        # 1. Channel cross-attention: Exchange global context (Eq. 172 & 173)
        ...
        # 2. Spatial cross-gating: Exchange local details
        # S_r, S_t: Depthwise separable feature extraction (Eq. 196 & 197)
        rgb_sp = self.rgb_spatial(rgb_enhanced)
        thermal_sp = self.thermal_spatial(thermal_enhanced)

        # Cross-gating calculations (Eq. 198 & 199)
        ...
        # Normalize and add residual connections (Eq. 200 & 201)
        rgb_out = self.norm_rgb(rgb_feat + rgb_cross)
        thermal_out = self.norm_thermal(thermal_feat + thermal_cross)

        # 3. Gated fusion: Learnable gate weight alpha (Eq. 211 - 213)
        gate = self.fusion_gate(torch.cat([rgb_out, thermal_out], dim=1))
        fused = gate * rgb_out + (1.0 - gate) * thermal_out

        # Final projection and residual wrapper (Eq. 215 & 216)
