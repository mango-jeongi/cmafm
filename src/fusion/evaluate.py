...
def _compute_precision_recall_f1(coco_eval: COCOeval, iou_thr: float = 0.5) -> Dict[str, float]:
    """
    Extract Precision, Recall, and F1 from COCOeval results.
...
@torch.no_grad()
def evaluate_late_fusion(model_rgb, model_thermal, data_loader: DataLoader,
                         device: torch.device, data_cfg: DataConfig,
                         nms_thresh: float = 0.5) -> Dict[str, float]:
    """
    Evaluate Late Fusion: runs independent RGB-only and Thermal-only models,
    combines all predictions, and applies batched NMS to resolve conflicts.

    Returns evaluation metrics identical to evaluate_model.
    """
    model_rgb.eval()
    model_thermal.eval()
    ...
    for rgb, thermal, targets in data_loader:
        ...
        t0 = time.perf_counter()
        # Single-modal models only run the forward pass on their respective modality internally
        outputs_rgb = model_rgb(rgb, thermal)
        outputs_thermal = model_thermal(rgb, thermal)
        if device.type == "cuda":
            torch.cuda.synchronize()
        total_time += time.perf_counter() - t0
        total_images += len(targets)

        for i, (out_r, out_t, target) in enumerate(zip(outputs_rgb, outputs_thermal, targets)):
            img_id = img_id_counter + i

            # Combine predictions
            boxes_r = out_r["boxes"]
            scores_r = out_r["scores"]
            labels_r = out_r["labels"]

            boxes_t = out_t["boxes"]
            scores_t = out_t["scores"]
            labels_t = out_t["labels"]

            combined_boxes = torch.cat([boxes_r, boxes_t], dim=0)
            combined_scores = torch.cat([scores_r, scores_t], dim=0)
            combined_labels = torch.cat([labels_r, labels_t], dim=0)

            # Apply batched NMS to ensemble overlapping bounding boxes
            keep = batched_nms(combined_boxes, combined_scores, combined_labels, nms_thresh)

            fused_boxes = combined_boxes[keep].cpu().numpy()
            fused_scores = combined_scores[keep].cpu().numpy()
            fused_labels = combined_labels[keep].cpu().numpy()
