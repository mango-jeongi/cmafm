...
# Load .env from the repository root (two levels up from src/fusion/)
_REPO_ROOT = _HERE.parent.parent
load_dotenv(_REPO_ROOT / ".env")

# Resolve data directory: prefer CMAFM_DATA_DIR env var, fall back to ../data
_DATA_DIR = Path(os.environ.get("CMAFM_DATA_DIR", str(_REPO_ROOT.parent / "data")))


@dataclass
class DataConfig:
    """Dataset configuration for M3FD."""
    root: str = str(_DATA_DIR / "M3FD")
...
class TrainConfig:
    ...
    # Checkpointing
    save_dir: str = str(_REPO_ROOT / "runs")
    save_every: int = 5
