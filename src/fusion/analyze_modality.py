"""
Quantitative evaluation script by modality: RGB-only / Thermal-only / Fused (RGB+Thermal).
Calculates complete metrics including Precision, Recall, F1, Miss Rate, FPS, and mAP.
"""
...
def run_eval(model, dataset, indices, device, cfg, label):
    ...
    print(f"    → Evaluating {label} ({len(subset)} images)...", flush=True)
    return evaluate_model(model, loader, device, cfg.data)


def print_metrics(r, label):
    print(f"  {'Metric':<14} {label}")
...
def main():
    ...
    print("Classifying night and day scenes...")
    night_idx, day_idx = classify_night_day(val_dataset, vis_dir)
    all_idx = list(range(len(val_dataset)))
    print(f"  Total: {len(all_idx)} / Night: {len(night_idx)} / Day: {len(day_idx)}\n")
    ...
    for display_name, key, ckpt_path, variant_name in variants:
        print("=" * 60)
        print(f"[{display_name}]  Checkpoint: {ckpt_path.name}")
        print("=" * 60)
        ...
        # Ablation checkpoints save state_dict directly without packaging dictionary
        ...
        # All
        cond_results["all"]   = run_eval(model, val_dataset, all_idx,   device, cfg, "All")
        print_metrics(cond_results["all"], "All")

        # Night
        cond_results["night"] = run_eval(model, val_dataset, night_idx, device, cfg, "Night")
        print_metrics(cond_results["night"], "Night")

        # Day
        cond_results["day"]   = run_eval(model, val_dataset, day_idx,   device, cfg, "Day")
        print_metrics(cond_results["day"], "Day")
    ...
    print("\n" + "=" * 90)
    print("Final Summary — Modality Performance (All / Night / Day)")
    print("=" * 90)
    metrics = [("mAP@0.5","mAP50"), ("Recall","recall"), ("F1","f1"), ("MissRate","miss_rate")]
    conds = [("All","all"), ("Night","night"), ("Day","day")]
