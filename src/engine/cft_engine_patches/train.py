...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        model = Model(opt.cfg or (ckpt['model'].yaml if 'model' in ckpt and hasattr(ckpt['model'], 'yaml') else None), ch=6, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        exclude = ['anchor'] if (opt.cfg or hyp.get('anchors')) and not opt.resume else []  # exclude keys
        
        # Robust weight extraction: handle full objects vs raw state dicts
        if isinstance(ckpt, dict) and 'model' in ckpt:
            # Check if it's a full Model object or a nested state dict
    ...
    if pretrained:
        ...
        # EMA
        if ema and isinstance(ckpt, dict) and ckpt.get('ema'):
            ema.ema.load_state_dict(ckpt['ema'].float().state_dict())
            ema.updates = ckpt['updates']

        # Results
        if isinstance(ckpt, dict) and ckpt.get('training_results') is not None:
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        if isinstance(ckpt, dict) and 'model' in ckpt:
            m_obj = ckpt['model']
            if hasattr(m_obj, 'float') and hasattr(m_obj, 'state_dict'):
                state_dict = m_obj.float().state_dict()
            else:
                state_dict = m_obj # Already a state dict
        else:
            state_dict = ckpt
...
if __name__ == '__main__':
    ...
    if opt.global_rank in [-1, 0]:
        # check_git_status()
        # check_requirements()  # Disabled for HPC stability
        pass
